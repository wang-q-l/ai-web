...
<script setup lang="ts">
  ...
  const shouldShow = computed(() => {
    ...
    // selector 类型且选择器找不到对应元素时不显示（避免在不相关 tab 上错位显示）
    if (props.annotation.type === 'selector' && !selectorPos.value) return false
  ...
  const categoryLabel = computed(
    () => categoryMap[props.annotation.category] || props.annotation.category
  )
  ...
  const getFallbackSelectors = (): string[] => {
    ...
    if (category === 'field') {
      ...
      if (t.includes('表单') || t.includes('form')) {
        return ['.ant-form', '.el-form', 'form', '[class*="form-card"]', '[class*="form_card"]']
      }
      ...
      if (t.includes('步骤') || t.includes('step')) {
        return ['.ant-steps', '.el-steps', '[class*="form-steps"]', '[class*="step"]']
      }
  ...
  const updateSelectorPos = () => {
    ...
    // 第二步：未配置 selector 时，根据 category + title 动态推断
...
<style scoped lang="scss">
  ...
  .annotation-dot-unplaced {
    ...
    bottom: 80px;
    left: auto !important;
    border: 2px dashed rgb(255 255 255 / 80%) !important;
    opacity: 0.85;
  }
