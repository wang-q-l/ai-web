    ...
    MENU_CONF: {
      uploadImage: {
        ...
        customInsert(res: any, insertFn: (url: string, alt: string, href: string) => void) {
          // 后端返回格式: { code: 200, data: { url: '/uploads/images/xxx.jpg' } }
          if (res.code === 200 && res.data?.url) {
            // 将相对路径转为完整URL
            const imageUrl = res.data.url.startsWith('http')
              ? res.data.url
              : `${import.meta.env.VITE_API_PROXY_URL}${res.data.url}`
            ...
            ElMessage.error(`图片上传失败：${res.message || '未知错误'} ${EmojiText[500]}`)
          }
        },

        onError(file: File, err: any, res: any) {
          console.error('图片上传请求失败:', err, res)
