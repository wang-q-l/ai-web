  ...
  <div class="page-container">
    ...
    <div class="page-header">
      ...
      <div class="header-right-area">
        <el-input v-model="searchKeyword" placeholder="项目名称搜索" style="width: 220px" clearable>
          <template #prefix>
    ...
    <el-card class="project-info-banner">
      <div class="banner-content">
        ...
        <div class="banner-item"
          ><i class="iconfont-sys banner-icon" v-html="'&#xe61b;'"></i
          ><span class="banner-label">审计组长</span
          ><span class="banner-value">{{ currentProject?.leader }}</span></div
        >
        <div class="banner-item"
          ><i class="iconfont-sys banner-icon" v-html="'&#xe625;'"></i
          ><span class="banner-label">项目经理</span
          ><span class="banner-value">{{ currentProject?.manager }}</span></div
        >
        <div class="banner-item"
          ><i class="iconfont-sys banner-icon" v-html="'&#xe88a;'"></i
          ><span class="banner-label">中介机构</span
          ><span class="banner-value">{{ currentProject?.organization }}</span></div
        >
        <div class="banner-item"
          ><i class="iconfont-sys banner-icon" v-html="'&#xe7ae;'"></i
          ><span class="banner-label">中介人员数量</span
          ><span class="banner-value">{{ currentProject?.userCount }}人</span></div
        >
      </div>
    </el-card>
          ...
          <template #header>
            <div class="card-header">
              <div class="header-left">
                ...
                <div class="legend-list">
                  ...
                  <span class="legend-item"
                    ><i class="legend-dot dot-insufficient"></i>工时不足</span
                  >
              ...
              <div class="header-right">
                <el-button type="primary" size="small" @click="handleCheckinToday"
                  >现场打卡</el-button
                >
                <el-button :icon="ArrowLeft" @click="handlePrevMonth">上个月</el-button>
                <span class="current-month">{{ currentMonthText }}</span>
                <el-button :icon="ArrowRight" @click="handleNextMonth" :disabled="isCurrentMonth"
                  >下个月</el-button
                >
              </div>
            </div>
                  ...
                  <tbody>
                    <tr v-for="user in filteredGanttData" :key="user.userId">
                      ...
                      >
                        ...
                        <!-- 休息日无数据 -->
                        <div
                          v-else-if="isWeekend(date) && !user.dateMap[date]"
                          class="status-block block-weekend"
                        ></div>
                        <!-- 有打卡记录 -->
                        <el-tooltip v-else-if="user.dateMap[date]" placement="top">
                          <template #content>
                            <div class="tooltip-content">
                              ...
                              <div
                                v-if="!user.dateMap[date].clockIn || !user.dateMap[date].clockOut"
                              >
                                <el-button
                                  link
                                  type="primary"
                                  size="small"
                                  @click.stop="handleMakeupCheckin(user, date)"
                                >
                                  申请补卡
                                </el-button>
                              </div>
          ...
          <div class="table-container">
            <el-table :data="filteredTableData" height="100%" width="100%">
              ...
              <el-table-column prop="clockIn" label="上班打卡" min-width="120">
                <template #default="{ row }">
                  <span :style="{ color: row.clockIn ? '#303133' : '#c0c4cc' }">{{
                    row.clockIn || '未打卡'
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="clockOut" label="下班打卡" min-width="120">
                <template #default="{ row }">
                  <span :style="{ color: row.clockOut ? '#303133' : '#c0c4cc' }">{{
                    row.clockOut || '未打卡'
                  }}</span>
                </template>
              </el-table-column>
          ...
          <template #header>
            <div class="card-header">
              ...
              <span class="card-subtitle"
                >请假申请需经审批后生效，审批通过后甘特图中将显示请假状态</span
              >
          ...
          <div class="table-container">
            <el-table :data="filteredLeaveRecords" height="100%" width="100%">
              ...
              <el-table-column prop="leaveType" label="请假类型" width="120">
                <template #default="{ row }">
                  <el-tag :type="leaveTypeTagMap[row.leaveType]" size="small">{{
                    row.leaveType
                  }}</el-tag>
                </template>
              </el-table-column>
          ...
          <div class="table-container">
            <el-table :data="filteredCheckinStatsData" height="100%" width="100%">
              ...
              <el-table-column prop="insufficientDays" label="工时不足天数" min-width="120">
                <template #default="{ row }">
                  <span :style="{ color: row.insufficientDays > 0 ? '#f56c6c' : '#303133' }">{{
                    row.insufficientDays
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="absentDays" label="缺勤天数" min-width="100">
                <template #default="{ row }">
                  <span :style="{ color: row.absentDays > 0 ? '#f56c6c' : '#303133' }">{{
                    row.absentDays
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="missingClockCount" label="漏打卡次数" min-width="110">
                <template #default="{ row }">
                  <span :style="{ color: row.missingClockCount > 0 ? '#e6a23c' : '#303133' }">{{
                    row.missingClockCount
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="makeupCount" label="补卡次数" min-width="100">
                <template #default="{ row }">
                  <span :style="{ color: row.makeupCount > 0 ? '#409eff' : '#303133' }">{{
                    row.makeupCount
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="leaveDays" label="请假天数" min-width="100">
                <template #default="{ row }">
                  <span :style="{ color: row.leaveDays > 0 ? '#9d65c9' : '#303133' }">{{
                    row.leaveDays
                  }}</span>
                </template>
              </el-table-column>
    ...
    >
      <el-form :model="leaveApproveForm" label-width="100px">
        ...
        <el-form-item label="请假日期">
          <el-input
            :value="`${leaveApproveForm.startDate} 至 ${leaveApproveForm.endDate}`"
            disabled
          />
        </el-form-item>
        <el-form-item label="审批结果">
...
<script setup lang="ts">
      ...
      if (res.code === 200) {
        ...
        res.data.userStats.forEach((user: any) => {
          ...
          const dateMap = new Map<
            string,
            { clockIn: string; clockOut: string; duration: string; hours: number }
          >()
    ...
    data.userStats.forEach((user) => {
      ...
      dateRecords.forEach((records, dateKey) => {
        ...
        if (clockInRecord || clockOutRecord) {
          const clockIn = clockInRecord
            ? clockInRecord.checkinTime.split(' ')[1].substring(0, 5)
            : ''
          const clockOut = clockOutRecord
            ? clockOutRecord.checkinTime.split(' ')[1].substring(0, 5)
            : ''
  ...
  const isOnLeave = (userId: number, date: string) => {
    return leaveRecords.value.some(
      (r) => r.userId === userId && r.status === 1 && date >= r.startDate && date <= r.endDate
    )
  }

  // 获取请假类型
  const getLeaveType = (userId: number, date: string) => {
    const record = leaveRecords.value.find(
      (r) => r.userId === userId && r.status === 1 && date >= r.startDate && date <= r.endDate
    )
