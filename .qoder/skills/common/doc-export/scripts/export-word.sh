#!/bin/bash
# 通用文档 → Word 导出脚本
# 用法：bash .claude/skills/common/doc-export/scripts/export-word.sh <输入.md> [输出.docx]

set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
COMMON_DIR="$(dirname "$SCRIPT_DIR")"
REFERENCE_DOC="$COMMON_DIR/assets/reference.docx"
LOG_FILE="/tmp/doc-export-$(date +%Y%m%d-%H%M%S).log"
